...
(async () => {
  let all_contacts = document.getElementById("all-contacts");
    ...
    if (response.status == 200) {
      ...
      res.data.map((data, index) => {
        ...
        <tr
          ...
          <td class="px-6 py-4 hover:text-blue-800 cursor-pointer " onclick="deleteContact('${data._id}')">Delete</td>
          </tr>
        `;
      });
    } else {
  ...
  deleteContact = async (id) => {
    try {
      const response = await fetch(`${api}/admin/deletecontact/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `token ${storage ? storage : ""}`,
